...
** Motivation
I wanted something like this specifically to enable better DX for Svelte in Emacs. lsp-mode has a client for Svelte, but without the TypeScript plugin (which enables finding references to symbols from JS/TS files in .svelte files), Svelte support was incomplete.
